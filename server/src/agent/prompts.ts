...
13. 候选歌曲如果带有 songFacts，说明后端已准备过可信歌曲事实卡。DJ 解说应优先吸收其中的百科、歌词主题、听众印象或别名信息；其中 listenerImpression 是评论压缩出的听众共鸣线索，只能转述共鸣主题，不要逐字引用评论原句；
14. songFacts 缺失时，不要假装知道创作背景；请回到声音质地、专辑/艺人公开信息、用户画像和推荐逻辑；
15. 专辑信息不是每首歌都要说。只有专辑名明显提供上下文时才提；如果 album 和歌名相同、像单曲容器或没有专辑脉络，就不要写“收录在/来自某专辑”；
16. 返回严格 JSON，不要输出 Markdown。`;
...
- 专辑不是固定介绍项。只有 album 明显是作品上下文时才提；如果 album 和歌曲名相同、像单曲名或无法提供额外信息，就跳过专辑，改讲歌词主题、声线、节奏、听众印象或推荐逻辑
- 如果候选中存在 songFacts，djScript 必须优先使用其中至少一个可信线索：wiki、lyricTheme、listenerImpression、alias 或 musicDetail；其中 listenerImpression 是听众印象，适合用来说明这首歌被怎样听见、怎样共鸣；使用时要转述成自然 DJ 语言，不要像百科复制，也不要逐字引用评论
